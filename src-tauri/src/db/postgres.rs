...
impl PostgresDatabase {
    ...
    pub fn new(connection_string: &str) -> Result<Self, String> {
        ...
        // Additional validation can be added here (e.g., ensuring password is not empty if required)
...
impl Database for PostgresDatabase {
    ...
    fn delete_product(&self, id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM products WHERE id = $1", &[&id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_employees(&self) -> Result<Vec<Employee>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, employee_id, first_name, last_name, email, phone, role, department, position, salary, status FROM employees", &[]).map_err(|e| e.to_string())?;
    ...
    fn delete_employee(&self, id: i32) -> Result<(), String> {
        ...
        if let Err(e) = tx.execute("UPDATE tasks SET employee_id = NULL WHERE employee_id = $1", &[&id]) { println!("Error updating tasks: {:?}", e); return Err(e.to_string()); }
        if let Err(e) = tx.execute("UPDATE project_tasks SET assigned_to = NULL WHERE assigned_to = $1", &[&id]) { println!("Error updating project_tasks: {:?}", e); return Err(e.to_string()); }
        if let Err(e) = tx.execute("UPDATE payments SET employee_id = NULL WHERE employee_id = $1", &[&id]) { println!("Error updating payments: {:?}", e); return Err(e.to_string()); }
        if let Err(e) = tx.execute("UPDATE projects SET manager_id = NULL WHERE manager_id = $1", &[&id]) { println!("Error updating projects: {:?}", e); return Err(e.to_string()); }
        
        // 1. Attendance deletion with SAVEPOINT
        if let Err(_) = tx.execute("SAVEPOINT attendance_del", &[]) { return Err("Failed to create savepoint".to_string()); }
        if let Err(e) = tx.execute("DELETE FROM attendance WHERE employee_id = $1", &[&id]) { 
            if e.code() == Some(&SqlState::UNDEFINED_TABLE) {
                println!("Attendance table missing, skipping deletion.");
                if let Err(_) = tx.execute("ROLLBACK TO SAVEPOINT attendance_del", &[]) { return Err("Failed to rollback savepoint".to_string()); }
            } else {
                println!("Error deleting attendance: {:?}", e);
                return Err(format!("Error deleting attendance: {:?}", e));
            }
        } else {
            tx.execute("RELEASE SAVEPOINT attendance_del", &[]).ok();
        }

        // 2. Tool Assignments deletion with SAVEPOINT
        if let Err(_) = tx.execute("SAVEPOINT tool_assign_del", &[]) { return Err("Failed to create savepoint".to_string()); }
        if let Err(e) = tx.execute("DELETE FROM tool_assignments WHERE employee_id = $1", &[&id]) { 
            if e.code() == Some(&SqlState::UNDEFINED_TABLE) {
                 println!("Tool assignments table missing, skipping deletion.");
                 if let Err(_) = tx.execute("ROLLBACK TO SAVEPOINT tool_assign_del", &[]) { return Err("Failed to rollback savepoint".to_string()); }
            } else {
                println!("Error deleting tool_assignments: {:?}", e); 
                return Err(e.to_string()); 
            }
        } else {
            tx.execute("RELEASE SAVEPOINT tool_assign_del", &[]).ok();
        }

        if let Err(e) = tx.execute("UPDATE tools SET assigned_to_employee_id = NULL WHERE assigned_to_employee_id = $1", &[&id]) { println!("Error updating tools: {:?}", e); return Err(e.to_string()); }
        
        // 3. Project Assignments deletion with SAVEPOINT
        if let Err(_) = tx.execute("SAVEPOINT project_assign_del", &[]) { return Err("Failed to create savepoint".to_string()); }
        if let Err(e) = tx.execute("DELETE FROM project_assignments WHERE employee_id = $1", &[&id]) { 
            if e.code() == Some(&SqlState::UNDEFINED_TABLE) {
                println!("Project assignments table missing, skipping deletion.");
                if let Err(_) = tx.execute("ROLLBACK TO SAVEPOINT project_assign_del", &[]) { return Err("Failed to rollback savepoint".to_string()); }
            } else {
                println!("Error deleting project_assignments: {:?}", e); 
                return Err(e.to_string()); 
            }
        } else {
            tx.execute("RELEASE SAVEPOINT project_assign_del", &[]).ok();
        }
        
        if let Err(e) = tx.execute("DELETE FROM employees WHERE id = $1", &[&id]) { 
    ...
    fn get_payments(&self) -> Result<Vec<Payment>, String> {
         let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
         let rows = client.query("SELECT id, payment_type, amount, currency, description, status, payment_method, payment_date, due_date, reference_number, employee_id, supplier_name FROM payments", &[]).map_err(|e| e.to_string())?;
    ...
    fn delete_payment(&self, id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM payments WHERE id = $1", &[&id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_tasks(&self) -> Result<Vec<Task>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, employee_id, title, description, due_date, status, priority, assigned_date, completed_date FROM tasks", &[]).map_err(|e| e.to_string())?;
    ...
    fn delete_task(&self, id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM tasks WHERE id = $1", &[&id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_attendances(&self) -> Result<Vec<Attendance>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, employee_id, check_in, check_out, status, notes, location FROM attendance", &[]).map_err(|e| e.to_string())?;
    ...
    fn get_dashboard_stats(&self) -> Result<DashboardStats, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let total_products: i64 = client.query_one("SELECT COUNT(*) FROM products", &[]).map_err(|e| e.to_string())?.get(0);
        let low_stock_items: i64 = client.query_one("SELECT COUNT(*) FROM products WHERE current_quantity <= minimum_quantity", &[]).map_err(|e| e.to_string())?.get(0);
        let total_employees: i64 = client.query_one("SELECT COUNT(*) FROM employees WHERE status = 'active'", &[]).map_err(|e| e.to_string())?.get(0);
        let total_payments_pending: i64 = client.query_one("SELECT COUNT(*) FROM payments WHERE status = 'pending'", &[]).map_err(|e| e.to_string())?.get(0);
    ...
    fn get_report_summary(&self) -> Result<ReportSummary, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let inventory_value: f64 = client.query_one("SELECT COALESCE(SUM(current_quantity * unit_price), 0.0) FROM products", &[]).map_err(|e| e.to_string())?.get(0);
        let total_revenue: f64 = client.query_one("SELECT COALESCE(SUM(amount), 0.0) FROM payments WHERE payment_type = 'income' AND status = 'completed'", &[]).map_err(|e| e.to_string())?.get(0);
        let total_expenses: f64 = client.query_one("SELECT COALESCE(SUM(amount), 0.0) FROM payments WHERE payment_type = 'expense' AND status = 'completed'", &[]).map_err(|e| e.to_string())?.get(0);
        let pending_tasks: i64 = client.query_one("SELECT COUNT(*) FROM tasks WHERE status != 'completed'", &[]).map_err(|e| e.to_string())?.get(0);
        let active_employees: i64 = client.query_one("SELECT COUNT(*) FROM employees WHERE status = 'active'", &[]).map_err(|e| e.to_string())?.get(0);
    ...
    fn get_complaints(&self) -> Result<Vec<Complaint>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, content, created_at, status, admin_notes, resolution, resolved_at, resolved_by FROM complaints", &[]).map_err(|e| e.to_string())?;
    ...
    fn submit_complaint(&self, complaint: Complaint) -> Result<i64, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let row = client.query_one("INSERT INTO complaints (content) VALUES ($1) RETURNING id", &[&complaint.content]).map_err(|e| e.to_string())?;
        Ok(row.get::<_, i32>(0) as i64)
    }
    ...
    fn delete_complaint(&self, id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM complaints WHERE id = $1", &[&id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_tools(&self) -> Result<Vec<Tool>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, name, type_name, status, assigned_to_employee_id, purchase_date, condition FROM tools", &[]).map_err(|e| e.to_string())?;
    ...
    fn delete_tool(&self, id: i32) -> Result<(), String> {
        ...
        if let Err(_) = tx.execute("SAVEPOINT tool_assign_del", &[]) { return Err("Failed to create savepoint".to_string()); }
        if let Err(e) = tx.execute("DELETE FROM tool_assignments WHERE tool_id = $1", &[&id]) {
            if e.code() == Some(&SqlState::UNDEFINED_TABLE) {
                println!("Tool assignments table missing, skipping deletion.");
                if let Err(_) = tx.execute("ROLLBACK TO SAVEPOINT tool_assign_del", &[]) { return Err("Failed to rollback savepoint".to_string()); }
            } else {
        ...
        if let Err(e) = tx.execute("DELETE FROM tools WHERE id = $1", &[&id]) {
    ...
    fn assign_tool(&self, assignment: ToolAssignment) -> Result<i64, String> {
        ...
        tx.execute("UPDATE tools SET assigned_to_employee_id = $1, status = 'assigned' WHERE id = $2", &[&assignment.employee_id, &assignment.tool_id]).map_err(|e| e.to_string())?;
    ...
    fn return_tool(&self, tool_id: i32, condition: String) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let mut tx = client.transaction().map_err(|e| e.to_string())?;
        tx.execute("UPDATE tools SET assigned_to_employee_id = NULL, status = 'available', condition = COALESCE($1, condition) WHERE id = $2", &[&condition, &tool_id]).map_err(|e| e.to_string())?;
        tx.execute("UPDATE tool_assignments SET returned_at = CURRENT_TIMESTAMP, condition_on_return = $1 WHERE tool_id = $2 AND returned_at IS NULL", &[&condition, &tool_id]).map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_tool_history(&self, tool_id: i32) -> Result<Vec<ToolAssignment>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, tool_id, employee_id, assigned_at, returned_at, condition_on_assignment, condition_on_return, notes FROM tool_assignments WHERE tool_id = $1 ORDER BY assigned_at DESC", &[&tool_id]).map_err(|e| e.to_string())?;
    ...
    fn get_roles(&self) -> Result<Vec<Role>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, name, description, is_custom FROM roles", &[]).map_err(|e| e.to_string())?;
    ...
    fn add_role(&self, role: Role) -> Result<i64, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let row = client.query_one("INSERT INTO roles (name, description, is_custom) VALUES ($1, $2, $3) RETURNING id", &[&role.name, &role.description, &role.is_custom]).map_err(|e| e.to_string())?;
        Ok(row.get::<_, i32>(0) as i64)
    }

    fn get_permissions(&self) -> Result<Vec<Permission>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, code, description FROM permissions", &[]).map_err(|e| e.to_string())?;
    ...
    fn get_role_permissions(&self, role_id: i32) -> Result<Vec<Permission>, String> {
        ...
        let query = "
            ...
            WHERE rp.role_id = $1
        ";
        let rows = client.query(query, &[&role_id]).map_err(|e| e.to_string())?;
        let mut permissions = Vec::new();
        for row in rows {
            permissions.push(Permission {
                id: row.get(0),
    ...
    fn update_role_permissions(&self, role_id: i32, permission_ids: Vec<i32>) -> Result<(), String> {
        ...
        for perm_id in permission_ids {
            tx.execute("INSERT INTO role_permissions (role_id, permission_id) VALUES ($1, $2)", &[&role_id, &perm_id]).map_err(|e| e.to_string())?;
        }
        tx.commit().map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_feature_toggles(&self) -> Result<Vec<FeatureToggle>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT key, is_enabled FROM feature_toggles", &[]).map_err(|e| e.to_string())?;
    ...
    fn set_feature_toggle(&self, key: String, is_enabled: bool) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("INSERT INTO feature_toggles (key, is_enabled) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET is_enabled = $2", &[&key, &is_enabled]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_setup_status(&self) -> Result<bool, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let row = client.query_opt("SELECT setup_completed FROM setup_config LIMIT 1", &[]).map_err(|e| e.to_string())?;
        if let Some(r) = row {
            Ok(r.get(0))
        } else {
            Ok(false)
    ...
    fn complete_setup(&self, company_name: String, admin_email: String, admin_password: String) -> Result<(), String> {
        ...
        // Assuming admin_password corresponds to license_key or similar credential in this schema context, 
        // or just storing it in license_key column for now as per previous implementation logic.
        client.execute("INSERT INTO setup_config (company_name, company_email, license_key, setup_completed, setup_completed_at) VALUES ($1, $2, $3, TRUE, $4)", &[&company_name, &admin_email, &admin_password, &setup_completed_at]).map_err(|e| e.to_string())?;
        Ok(())
    }

    fn set_company_name(&self, company_name: String) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("UPDATE setup_config SET company_name = $1", &[&company_name]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_audit_logs(&self) -> Result<Vec<AuditLog>, String> {
        ...
        let rows = client.query("SELECT id, user_id, action, entity, entity_id, details, created_at FROM audit_logs ORDER BY created_at DESC LIMIT 100", &[]).map_err(|_| "Audit logs table not found".to_string())?;
    ...
    fn log_activity(&self, user_id: Option<i32>, action: String, entity: Option<String>, entity_id: Option<i32>, details: Option<String>) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("INSERT INTO audit_logs (user_id, action, entity, entity_id, details) VALUES ($1, $2, $3, $4, $5)", &[&user_id, &action, &entity, &entity_id, &details]).map_err(|_| "Failed to log".to_string())?;
        Ok(())
    }
    ...
    fn get_dashboard_configs(&self) -> Result<Vec<DashboardConfig>, String> {
        ...
        let rows = client.query("SELECT id, user_id, name, layout_json, is_default FROM dashboard_configs", &[]).map_err(|_| "Table not found".to_string())?;
    ...
    fn save_dashboard_config(&self, config: DashboardConfig) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("INSERT INTO dashboard_configs (user_id, name, layout_json, is_default) VALUES ($1, $2, $3, $4)", &[&config.user_id, &config.name, &config.layout_json, &config.is_default]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_projects(&self) -> Result<Vec<Project>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, name, description, start_date, end_date, status, manager_id FROM projects", &[]).map_err(|e| e.to_string())?;
    ...
    fn delete_project(&self, id: i32) -> Result<(), String> {
        ...
        tx.execute("DELETE FROM project_tasks WHERE project_id = $1", &[&id]).map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM project_assignments WHERE project_id = $1", &[&id]).map_err(|e| e.to_string())?;
    ...
    fn get_project_tasks(&self, project_id: i32) -> Result<Vec<ProjectTask>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, project_id, name, description, assigned_to, status, priority, start_date, due_date FROM project_tasks WHERE project_id = $1", &[&project_id]).map_err(|e| e.to_string())?;
        let mut tasks = Vec::new();
        for row in rows {
    ...
    fn delete_project_task(&self, id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM project_tasks WHERE id = $1", &[&id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_project_assignments(&self, project_id: i32) -> Result<Vec<ProjectAssignment>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, project_id, employee_id, role, assigned_at FROM project_assignments WHERE project_id = $1", &[&project_id]).map_err(|e| e.to_string())?;
    ...
    fn get_all_project_assignments(&self) -> Result<Vec<ProjectAssignment>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, project_id, employee_id, role, assigned_at FROM project_assignments", &[]).map_err(|e| e.to_string())?;
        let mut assignments = Vec::new();
        for row in rows {
            assignments.push(ProjectAssignment {
                id: Some(row.get(0)),
    ...
    fn remove_project_assignment(&self, project_id: i32, employee_id: i32) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("DELETE FROM project_assignments WHERE project_id = $1 AND employee_id = $2", &[&project_id, &employee_id]).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn get_accounts(&self) -> Result<Vec<Account>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, code, name, type, currency, is_active FROM accounts", &[]).map_err(|_| "Table not found".to_string())?;
    ...
    fn get_invoices(&self) -> Result<Vec<Invoice>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, customer_name, customer_email, invoice_date, due_date, total_amount, status, currency FROM invoices", &[]).map_err(|_| "Table not found".to_string())?;
    ...
    fn get_integrations(&self) -> Result<Vec<Integration>, String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        let rows = client.query("SELECT id, name, is_connected, api_key, config_json, connected_at FROM integrations", &[]).map_err(|_| "Table not found".to_string())?;
    ...
    fn toggle_integration(&self, id: i32, is_connected: bool) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("UPDATE integrations SET is_connected = $1 WHERE id = $2", &[&is_connected, &id]).map_err(|e| e.to_string())?;
        Ok(())
    }

    fn configure_integration(&self, id: i32, _api_key: Option<String>, config_json: Option<String>) -> Result<(), String> {
        let mut client = self.client.lock().map_err(|_| "Failed to lock db".to_string())?;
        client.execute("UPDATE integrations SET config_json = $1 WHERE id = $2", &[&config_json, &id]).map_err(|e| e.to_string())?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_delete_employee_and_tool() {
        ...
        {
            let mut client = db.client.lock().unwrap();
            client.execute("DELETE FROM tool_assignments WHERE employee_id IN (SELECT id FROM employees WHERE employee_id = 'TEST001')", &[]).ok();
            client.execute("DELETE FROM tools WHERE assigned_to_employee_id IN (SELECT id FROM employees WHERE employee_id = 'TEST001')", &[]).ok();
            client.execute("DELETE FROM employees WHERE employee_id = 'TEST001'", &[]).ok();
        }
        ...
        let emp_id = db.add_employee(emp).expect("Failed to add employee");
        ...
        let tool_id = db.add_tool(tool).expect("Failed to add tool");
        println!("Created tool: {}", tool_id);

        // 3. Delete Tool
        match db.delete_tool(tool_id as i32) {
            Ok(_) => println!("Tool deleted successfully"),
            Err(e) => println!("Failed to delete tool: {}", e),
        }

        // 4. Delete Employee
        match db.delete_employee(emp_id as i32) {
