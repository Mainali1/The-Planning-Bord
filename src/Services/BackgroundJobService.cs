...
namespace ThePlanningBord.Services
{
    public class BackgroundJobService : IDisposable
    {
        ...
        {
            if (!await _jobSemaphore.WaitAsync(0))
            {
                // Previous cycle still running; skip this tick to avoid duplicate processing.
                return;
            }
            ...
            try
            {
                ...
                if (!emailEnabled)
                {
                    ...
                    await _jsRuntime.InvokeVoidAsync("notify", "Inventory Alert", message);
                    return;
                }
                        ...
                        if (supplier != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(targetEmail))
                            {
                                ...
                                var po = new PurchaseOrder
                                {
                                    ...
                                    TotalAmount = item.ReorderQuantity * (item.CostPrice ?? item.UnitPrice), // Approx using purchase cost when available
                                    ...
                                    Lines = new List<PurchaseOrderLine>
                                    {
                                        new PurchaseOrderLine
                                        {
                                            ...
                                            TotalPrice = item.ReorderQuantity * (item.CostPrice ?? item.UnitPrice)
                                        }
                                    }
                                };
                ...
                if (autoOrderedItems.Any())
                {
                    await _jsRuntime.InvokeVoidAsync("notify", "Auto-Order", $"Auto-ordered {autoOrderedItems.Count} items.");
                }
            }
            catch (Exception ex)
            {
